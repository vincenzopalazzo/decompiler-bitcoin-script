...
// The actual version of the script doesn't support taproot,
// the witness address are not supported and the parser set the
// type of the address to NO standard.
// The only change to support taproot is to check also for the version 1 inside
// the method isP2WPKH and isP2WSH.
...
int main(int argc, char* argv[]) {
  ...
  if (optCode == "OP_HASH160") {
    ...
    if (optCodeEnd == "OP_EQUAL") {
      cout << "\n\t----------------------------------------| Results "
              "|-----------------------------------"
           << endl;
      cout << "\t                               ###  Script PAY TO SCRIPT HASH "
              " ###                            "
           << endl;
    ...
    if (optCodeHash == "OP_HASH160") {
      string optCodeKey = hex.substr(
          6,
          hex.length() - 10);  // meno i 6 tolti all'inzio, meno i 4 alla fine,
                               // attenzione anche al operatore OP_PUSHBYTES_20
      // string optCodeKey =
      ...
      if (optCodepenultimoOpt == "OP_EQUALVERIFY") {
        ...
        if (optCodeendOpt == "OP_CHECKSIG") {
          ...
          /*
                        Sha256Hash hashP2sh = Sha256::getHash(bytes.data(),
             bytes.size());

                        uint8_t resultHas160[Ripemd160::HASH_LEN];
                        Ripemd160::getHash(hashP2sh.value,
             sizeof(hashP2sh.value), resultHas160); vector<uint8_t>
             vectorHash160(resultHas160);*/
          ...
          cout << "\t----------------------------------------| Results "
                  "|-----------------------------------"
               << endl;
          cout << "\t                               ###  Script PAY TO PUBLIC "
                  "KEY HASH  ###                            "
               << endl;
          cout << "\t  Raw Script PubKey: " << hex << endl << endl;
          cout << "\t" << optCode << " " << optCodeHash << " " << optCodeKey
               << " " << optCodepenultimoOpt << " " << optCodeendOpt << endl;
          cout << "\t                                                          "
                  "                            "
               << endl;
  ...
  } else if ((optValue != std::stoul("0x00", nullptr, 16)) &&
             (optValue != std::stoul("0x76", nullptr, 16))) {
    ...
    auto version = std::atoi(
        hex.substr(0, 2)
            .c_str());  // This is the value of OP_0 that is the version of it
    auto opt_version = std::stoul(hex.substr(0, 2).c_str(), nullptr, 16);
    auto opt_push = std::stoul(hex.substr(2, 4).c_str(), nullptr, 16);

    Bytes bytes = hexBytes(hex.substr(4, hex.length()).c_str());
    auto address = segwit_addr::encode("bc", version, bytes);
    // The encoder of bech32 that return an empty string if there are errors.
    assert(!address.empty());

    auto opcode_version = bitcoinOpCode.opCodeList.find(opt_version);
    assert(opcode_version != bitcoinOpCode.opCodeList.end());

    // TODO: Find the correct OPCODE
    // auto opcode_push = bitcoinOpCode.opCodeList.find(opt_push);
    // assert(opcode_push != bitcoinOpCode.opCodeList.end());

    cout << "\t-------------------------------------------------| Results "
            "|---------------------------------------------"
         << endl;
    cout << "\t                                 ###  Script PAY TO WITNESS "
            "PUBLIC KEY HASH  ###                          "
         << endl;
    cout << "\t" << opcode_version->second << "(missing opcode push)"
         << hex.substr(2, hex.length()) << endl;
    cout << endl;
    cout << "\t The address: " << address << endl;
    cout << endl;
    cout << "\t https://blockstream.info/address/" << address << endl;
...
// example 0020e5736bf12976cc710b9833aac2e3333a5a5e9f24ac87ce49e868b496b5086023
bool isP2WSH(std::string hexScript) {
  // Not work look the scrip python
  return ((hexScript.length() == 34 * 2) &&
          (std::stoul(hexScript.substr(0, 2), nullptr, 16) ==
           std::stoul("0x00", nullptr, 16)));
}
...
std::string ToString(uint8_t bytes[Ripemd160::HASH_LEN]) {
  ...
  for (int i = 0; i < Ripemd160::HASH_LEN; i++) {
    ...
    stream << std::hex << std::setprecision(2) << std::setw(2)
           << std::setfill('0') << valueInt;
  }
